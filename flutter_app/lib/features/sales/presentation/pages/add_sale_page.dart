...
class _AddSalePageState extends State<AddSalePage> {
  ...
  void initState() {
    ...
    // Add a test customer for debugging
      ...
      if (response['success']) {
        ...
        try {
          final customers = customersData
              .map((json) {
                // For dropdown data, we only have basic fields, so we need to handle missing fields
          ...
          if (mounted) {
            ...
            _showErrorSnackBar('Failed to parse customer data: $parseError');
          }
        }
      } else {
        if (mounted) {
          ...
          _showErrorSnackBar('Failed to load customers: ${response['message']}');
        }
      }
    } catch (e) {
      if (mounted) {
  ...
  Future<void> _loadProducts() async {
    ...
    try {
      ...
      if (response['success']) {
        ...
        final allProducts = productsData.map((json) {
        ...
        if (mounted) {
          setState(() {
            _availableProducts = activeProducts;
          });
        }
      } else {
        if (mounted) {
          ...
          _showErrorSnackBar('Failed to load products: ${response['message']}');
        }
      }
    } catch (e) {
      if (mounted) {
  ...
  Future<void> _submitForm() async {
    ...
    try {
      ...
      if (!mounted) return;
      _showErrorSnackBar('Error creating sale: $e');
    } finally {
      if (mounted) {
        setState(() {
          _isLoading = false;
          ...
          child: Column(
            ...
            children: [
              ...
              SizedBox(
                ...
                child: AppButton(
                  text: _isLoading ? 'Creating Sale...' : 'Create Sale',
...
class _AddSaleItemDialogState extends State<_AddSaleItemDialog> {
  ...
  void _submitForm() {
    ...
    if (quantity > _selectedProduct!.stockQuantity) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Insufficient stock. Available: ${_selectedProduct!.stockQuantity}'),
